	...
	struct kn_tx_dispatch_result *result)
{
	...
	for (i = 0; i < queue->count && result->attempted < limit; i++) {
		...
		if (target == NULL || target->test_safe == 0 ||
		    target->enabled == 0 || target->open == 0 ||
		    target->memory == NULL || target->memory->open == 0) {
			frame->status = KN_TX_FRAME_FAILED;
			frame->last_error = KN_TX_DISPATCH_ERR_NO_SAFE_TARGET;
		...
		if (write_rc == KN_TRANSPORT_MEMORY_OK) {
			...
			dispatcher->stats.bytes_written +=
			    (uint64_t)frame->kiss_len;
			result->sent++;
	...
	if (result->attempted == 0 && queue->count != 0 && port_name == NULL &&
	    saw_safe_target == 0)
		return KN_TX_DISPATCH_OK;
	return KN_TX_DISPATCH_OK;
}
