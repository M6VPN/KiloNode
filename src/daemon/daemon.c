...
kn_daemon_run_foreground(const struct kn_config *config)
{
	...
	if (kn_rf_command_queue_init(&rf_commands,
	    config->rf_command.max_events) != KN_RF_COMMAND_QUEUE_OK)
		return KN_DAEMON_ERR_CONFIG;
	kn_rf_abuse_init(&rf_abuse, KN_RF_ABUSE_MAX_SOURCES);
	kn_rf_ignore_init(&rf_ignore);
	if (config->rf_command.has_ignore_list_path != 0 &&
	    kn_rf_ignore_load_file(&rf_ignore,
	    config->rf_command.ignore_list_path) != KN_RF_IGNORE_OK)
		return KN_DAEMON_ERR_CONFIG;
...
pop_frames(struct daemon_port *port, struct kn_daemon_stats *daemon_stats,
	...
	const struct kn_config *config, const struct daemon_port *ports,
	...
	const struct kn_rf_ignore_list *rf_ignore)
{
	...
	struct kn_port_stats port_stats[KN_CONFIG_PORT_MAX];
	const struct kn_heard_entry *heard_entries;
	size_t heard_count;
	size_t i;
	uint64_t command_id;
	...
	while (kn_kiss_stream_has_frame(&port->parser) != 0) {
		...
		if (frame.command == 0) {
			...
			if (ax25_rc == KN_AX25_OK) {
				...
				rx_event_id = rx_events != NULL &&
				    rx_events->enabled != 0 ?
				    kn_rx_queue_reserve_id(rx_events) : 0;
				if (kn_rx_event_from_ax25(&event, rx_event_id,
				    now, port->config->name, frame.port,
				    frame.command, &ax25,
				    rx_events != NULL ? rx_events->preview_bytes :
				...
				if (config != NULL && rf_commands != NULL &&
					...
					for (i = 0; i < port_count; i++)
						port_stats[i] = ports[i].stats;
					command_id =
					    kn_rf_command_queue_reserve_id(
					    rf_commands);
					rf_rc = kn_rf_command_from_rx(&command,
					    command_id, now, config, port_stats,
					    port_count, &event, ax25.payload,
					    ax25.payload_len);
					if (rf_rc == KN_RF_COMMAND_OK) {
						command_pushed = 0;
						reason[0] = '\0';
						abuse_rc =
						    kn_rf_abuse_check_command(
						    rf_abuse,
						    &config->rf_command,
						    rf_ignore,
						    &command.source, now,
						...
						if (abuse_rc ==
							...
							command.status = abuse_rc ==
							    KN_RF_ABUSE_ERR_IGNORED ?
							    KN_RF_COMMAND_STATUS_IGNORED :
							    KN_RF_COMMAND_STATUS_RATE_LIMITED;
							(void)snprintf(command.error,
							    sizeof(command.error),
							    "%s", reason);
							(void)kn_rf_abuse_record_rejected(
							    rf_abuse,
							    &config->rf_command,
							    &command.source, now,
							    command.error);
							(void)kn_rf_command_queue_push(
							    rf_commands,
							    &command);
							command_pushed = 1;
						}
						if (command_pushed == 0 &&
						    command.status ==
						    KN_RF_COMMAND_STATUS_OK)
							(void)
							    kn_rf_abuse_record_accepted(
							    rf_abuse,
							    &command.source, now);
						else if (command_pushed == 0)
							(void)
							    kn_rf_abuse_record_rejected(
							    rf_abuse,
							    &config->rf_command,
							    &command.source, now,
							    kn_rf_command_status_string(
							    command.status));
						if (command_pushed == 0 &&
						    config->rf_command.reply_enabled !=
						    0) {
							heard_entries =
							    kn_heard_entries(heard);
							heard_count =
							    kn_heard_count(heard);
							tx_frame_id = 0;
							reason[0] = '\0';
							abuse_rc =
							    kn_rf_abuse_check_reply(
							    rf_abuse,
							    &config->rf_command,
							    &command.source,
							    now, reason,
							...
							if (command_pushed == 0 &&
							    ...
							    sizeof(reason)) ==
							    KN_RF_REPLY_OK) {
								command.reply_queued = 1;
								command.tx_frame_id =
								    tx_frame_id;
								(void)kn_rf_abuse_record_reply(
								    rf_abuse,
								    &command.source,
								    now);
							} else if (reason[0] !=
