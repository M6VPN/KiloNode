...
- reassembled payload counter
- segment counters
- bounded delivered-payload diagnostics queue
- bounded reassembly diagnostics queue
- last state
- last error
...
- `local-disconnect`
- `send-i text=<payload>`
- `send-i text=<payload> segment=true`
...
Segmented `send-i` uses paclen from the endpoint AX.25 params. M2.3 sends one
segment at a time and requires RR before sending the next segment.
...
After a segmented payload completes, the endpoint records one reassembly
diagnostic. The reassembled payload is still diagnostics-only.
